...
tt_uint16_t const g_crc16_table[] =
{
...
tt_uint16_t const g_crc16_ccitt_table[] =
{
...
tt_uint16_t tt_crc16_make(tt_byte_t const* data, tt_size_t size, tt_uint16_t seed)
{
    ...
    tt_uint16_t const*  pt = (tt_uint16_t const*)g_crc16_table;
    while (data < ie) crc = pt[((tt_uint8_t)crc) ^ *data++] ^ (crc >> 8);

    // ok?
    return (tt_uint16_t)crc;
}
tt_uint16_t tt_crc16_make_from_cstr(tt_char_t const* cstr, tt_uint16_t seed)
{
    ...
    // make it
    return tt_crc16_make((tt_byte_t const*)cstr, strlen(cstr) + 1, seed);
}
tt_uint16_t tt_crc16_ccitt_make(tt_byte_t const* data, tt_size_t size, tt_uint16_t seed)
{
    ...
    // init value
    tt_uint16_t crc = seed;

    // done
    tt_byte_t const*    ie = data + size;
    tt_uint16_t const*  pt = (tt_uint16_t const*)g_crc16_ccitt_table;
    while (data < ie) crc = pt[((tt_uint8_t)crc) ^ *data++] ^ (crc >> 8);
...
tt_uint16_t tt_crc16_ccitt_make_from_cstr(tt_char_t const* cstr, tt_uint16_t seed)
{
    ...
    // make it
    return tt_crc16_ccitt_make((tt_byte_t const*)cstr, strlen(cstr) + 1, seed);
}
